<?php
    ...
    if(!$num_match){    //회원이 아닌 아이디를 입력한 상태
        ...
        if($pass != $db_pass){      //로그인 페이지로부터 입력한 패스워드와 회원가입 시 작성한 DB 내의 패스워드가 일치하지 않다면 -> 로그인 불가
            echo ("
                <script>
            ...
            ");
            //경고창을 띄우고나서 지금 현재 페이지로부터 이전 페이지로 이동을 시켜라. => [url 창] login_form.php로 변경되면서 로그인 화면이 다시 나온다.
            exit;   //탈출(이후 문구는 로딩하지마!).함수문에서 비교 했을 때, return; 과 유사
            ...
            //key1 : value1  ==>  userid : USER유저의 실제 아이디 데이터
            //key2 : value2  ==>  username : USER유저의 실제 이름 데이터
            //key3 : value3  ==>  userlevel : USER유저의 실제 레벨 데이터
            //key4 : value4  ==>  userpoint : USER유저의 실제 포인트 데이터
