<?php
    ...
    //제목과 내용 작성간 ''(작은 따옴표)를 기입한 경우, DB로 전송 불가
    $subject = str_replace("'","&#39;",$_POST["subject"]);

    //textarea에서 예시글로 "700자 이내로 작성해주세요."라는 문구에 근거해서 통제 시스템을 구축
    ...
    /*
        array(5) {
            ["name"]=> string(13) "003-heart.png"
            ["type"]=> string(9) "image/png"
            ["tmp_name"]=> string(49) "C:\Bitnami\wampstack-8.0.11-0\php\tmp\php5811.tmp"
            ["error"]=> int(0)
            ["size"]=> int(13840)
        }
    */
    ...
    $upfile_error = $_FILES["upfile"]["error"];   //파일 에러사항(예시, 파일이 깨진 경우)

    /*
        var_dump($upfile_name);     //string(13) "003-heart.png"
        var_dump($upfile_tmp_name); //string(49) "C:\Bitnami\wampstack-8.0.11-0\php\tmp\phpDB34.tmp"
        var_dump($upfile_type);     //string(9) "image/png"
        var_dump($upfile_size);     //int(13840)
        var_dump($upfile_error);    //int(0) => 에러없음
    */
    if($upfile_name && !$upfile_error){ //첨부파일의 이름이 존재하고, 에러가 없다면
        $lastPointIndexNum = strrpos($upfile_name, ".");    //003-heart.png 파일 이름을 가진 파일을 첨부파일로 넣었을 때,

        // var_dump($lastPointIndexNum);   //int(9)

        $file_replace = substr_replace($upfile_name, "|", $lastPointIndexNum, 1);
        // var_dump($file_replace);

        $file = explode("|", $file_replace);
        // var_dump($file);

        // $file = strrpos($upfile_name, "."); //jquery.min.js 파일 이름을 가진 파일을 첨부파일로 넣었을 때, strrpos("문자열 대상", "지정문자") : 문자열 마지막부터 지정문자를 찾아서 해당하는 곳의 인덱스번호(좌측으로부터 0)를 반환시킴
        //[javascript] lastIndexOf
        // var_dump($file);    //int(9) => 마지막부터 포인트를 찾아가는 과정에서 맨처음 나온 포인트는 10번 인덱스에 존재한다.

        /*
        $file = explode(".", $upfile_name);   //지정한 문자를 기준으로 문자열을 분리하여 배열화시킨다.
        var_dump($file);    //array(2) { [0]=> string(9) "003-heart" [1]=> string(3) "png" }
        */
        $file_name = $file[0];  //[0]=> string(9) "003-heart"
        ...
        // var_dump($new_file_name);   //string(19) "2021_10_22_16_24_02"

        $copied_file_name = $new_file_name.".".$file_ext;   //2021_10_22_15_39_30.png
        $uploaded_file = $upload_dir.$copied_file_name;
        // var_dump($uploaded_file);   //string(30) "./data/2021_10_22_16_24_02.png"
