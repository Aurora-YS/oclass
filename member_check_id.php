    ...
    <div id="idChk_txt">
        <?php
            ...
            if(!$id){
                ...
                //아이디 입력상자에 어떠한 값이라도 입력한 후에 "중복체크"라는 버튼을 클릭했을 때
                //http://localhost/oclass/member_check_id.php?id=abc

                include "./db_con.php";

                $sql = "select * from members where id='$id'";
                //기존 DB에 동일한 id가 존재하는 지를 찾는다. -> 만약 존재한다면 "아이디가 중복되었습니다."라는 문구를 띄워야 함. 다시 다른 아이디로 작성하도록 유도
                ...
                //mysqli_query(접속정보, 명령 체계를 포함한 문자데이터) : 1차 관문(접속 정보의 유효성을 검사) -> 2차 관문(sql 명령)
                ...
                if($num_record){
                    echo "<p><b>".$id."</b> 아이디는 중복된 아이디입니다.</p><p>다른 아이디를 사용해주세요.</p>";
                }else{
                    echo "<p><b>".$id."</b> 사용 가능한 아이디입니다.</p>";
                }
                mysqli_close($con);
            }
        ?>
