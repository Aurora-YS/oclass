            ...
            <div id="message_list">
                <ul id="message">
                    ...
                    <?php
                        ...
                        //총 11개 였던 메시지가 2번째 페이지의 첫번째 리스트 상세보기에서 삭제 -> 삭제 처리 후 리스트로 돌아오는 과정에서 2번째로 돌아왔더니 빈 리스트 페이지만 존재한다. -> 리스트 항목들이 존재하는 1번 페이지로 돌려놓는다.
                        /*
                            마지막 메시지를 삭제한 상태
                            - $total_record (현재 메시지의 전체 개수를 알고 있음)
                            - 두번째 페이지로 열어라~~~ page=$page(2) ==> 아무 리스트도 없는 상태
                            - 아무것도 없는 상태인데 2번 페이지가 열렸는데 중간에 없다는 것을 제어하는 부분이 존재
                            => for문에서 반복을 진행하지 않았다는 것을 알 수 있음
                            $start == $total_record
